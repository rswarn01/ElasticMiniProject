...
def insert():
    ...
    try:
        ...
        return Response.failure(
            500, "Something wrong, can't insert given data", payload=str(e)
        )
...
def insert_bulk_data(file):
    ...
    for row in json_records:
        record = {"_op_type": "index", "_index": index_name, "_source": row}
...
def search(args):
    try:
        ...
        page = 1
        per_page = 20
        # body = {"query": {"match": {"supplier_name": f"*{args['searching_data']}*"}}}
        ...
        #                     "multi_match": {
        #                         "query": f"*{args['searching_data']}*",
        #                         "fields": ["supplier_name", "contact_name"],
        #                     }
        #                 }
        #             ]
        #         }
        ...
        body = {
            "query": {
                "query_string": {
                    "query": f"*{args['searching_data']}*",
                    "fields": ["supplier_name", "contact_name", "designation"],
                }
            },
            "from": (page - 1) * per_page,
            "size": per_page,
...
def search_twitter(args):
    try:
        ...
        results = es.search(index="twitter_data", body=body)
        response = results["hits"]
        # pagination=results["hits"]["total"]
        return Response.success(response, pagination=results["hits"]["total"])
    except Exception as e:
...
def add_new_suppliers_generate_ingest_file(input_sheet):
    ...
    try:
        ...
        distinct_suppliers = new_df.drop_duplicates(subset=["Supplier Name"])
        if not supplier_df.empty:
...
def fill_additional_attribute(input_sheet):
    ...
    try:
        ...
        db.session.bulk_insert_mappings(SupplierContact, new_df.to_dict("records"))
        # upload a separate file for supplier code and Name combination.
        db.session.commit()
        return Response.success("Data filled")
...
def create_new_index_dummy(args):
    try:
        query = (
            db.session.query(SupplierContact, SupplierMaster)
            .join(
                SupplierContact,
                SupplierContact.supplier_id == SupplierMaster.supplier_id,
            )
            .with_entities(
                SupplierMaster.supplier_id,
                SupplierMaster.supplier_name,
                SupplierMaster.is_active.label("supplier_is_active"),
                SupplierMaster.rating,
                SupplierMaster.job_id,
                SupplierContact.supplier_contact_id,
                SupplierContact.contact_name,
                SupplierContact.email_address,
                SupplierContact.contact_number,
                SupplierContact.designation,
                SupplierContact.uploaded_by.label("supplier_contact_uploaded_by"),
                SupplierContact.is_active.label("supplier_contact_is_active"),
            )
        )

        all_data = query.filter(
...
def update_delete_document(args):
    try:
        ...
        es.update(
            index="supplier_additonal_attribute",
            id="hzwxFIMBDiasRSYAT06Y",
...
def sync_db_and_elastic(args):
    try:
        ...
        SupplierAdditionalAttribute.query.filter(
            SupplierAdditionalAttribute.supplier_attribute_id
            == args["supplier_attribute_id"]
